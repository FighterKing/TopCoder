...
public:
  int theMaxFlowers(vector <string> flowers, int r, int c) {
    ...
    if (r > 0) {
      for (int i = 0; i < r; ++i)
        sum += count(flowers[i].begin(), flowers[i].end(), 'F');
    } 
    if (sum > max) 
      max = sum;
    ...
    if (c > 0) {
      ...
      for (int i = 0; i < flowers.size(); ++i) 
        sum += count(flowers[i].begin(), flowers[i].begin() + c, 'F');
    }
    if (sum > max) 
      max = sum;

    if (c < flowers[0].size() - 1) {
      sum = 0;
      for (int i = 0; i < flowers.size(); ++i)
        sum += count(flowers[i].begin() + c + 1, flowers[i].end(), 'F');
    }
    if (sum > max) 
      max = sum;
