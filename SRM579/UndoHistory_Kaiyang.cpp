// Same solution as tc
// This problem is not hard, but hard to understand correctly.
#include <vector>
#include <list>
#include <map>
#include <set>
...
public:
  int minPresses(vector <string> lines) {
    ...
    for (; i < lines.size(); ++i) {
      ...
      toAdd = min(toAdd, static_cast<unsigned int>(2 + t.size() - (j - 1)));
      count += toAdd;
      // press enter;
      ++count;
      for (int z = 1; z <= t.size(); ++z)
        undo.insert(t.substr(0, z));
      text = t;
    }
    return count;
  }
