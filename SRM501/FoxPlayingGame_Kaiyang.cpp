...
public:
  double theMax(int nA, int nB, int paramA, int paramB) {
    ...
    if (scoreA > 0 && scoreB > 1) { 
      // first add then mul
      best = nA * scoreA * pow(scoreB, nB);
    } else if (scoreA > 0 && scoreB >= 0 && scoreB <= 1) {
      // first mul then add
      best = nA * scoreA;
    } else if (scoreA > 0 && scoreB <= -1) {
      if (nB % 2 == 0) {
        // first add then mul
        best = nA * scoreA * pow(scoreB, nB);
      } else {
        // one mul then add, then mul
        best = nA * scoreA * pow(scoreB, nB - 1);
      }
    } else if (scoreA > 0 && scoreB > -1 && scoreB < 0) {
          best = nA * scoreA;
    } else if (scoreA == 0) {
      return 0;
    } else if (scoreA < 0 && scoreB > 1) {
      ...
      if (nB % 2 == 0) {
        if (nB == 0) best = nA * scoreA;
        else best = nA * scoreA * pow(scoreB, nB - 1);
      } else {
        best = nA * scoreA * pow(scoreB, nB);
      }
    } else if (scoreA < 0 && scoreB > -1 && scoreB < 0) {
      if (nB % 2 == 0) {
        if (nB == 0) best = nA * scoreA;
        else best = nA * scoreA * scoreB;
      } else {
        best = nA * scoreA * scoreB;
      }
    }
    return best;
  }
