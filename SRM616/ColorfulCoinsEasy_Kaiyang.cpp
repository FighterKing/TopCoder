// The same solution as explained from tc;
// let me explain the solution with a simple example.
// for example {1, 5, 15, 90}
// the number of 1 coin: 1~4((5 - 1) / 1)
// the number of 5 coin: 1~2((15 - 1) / 5)
// the number of 15 coin: 1~5((90 - 1) / 15)
// the number of 90 coin: 1~inf
// then one solution, we  can pick one 5 coin, two 1 coin, three 15 coin, and any number 90 coin.
#include <vector>
#include <list>
#include <map>
#include <set>
...
public:
  string isPossible(vector <int> values) {
    vector<int> ranges(values.size() - 1, 0);
    int i = 0; 
    for (; i < values.size() - 1; ++i) 
      ranges[i] = (values[i + 1] - 1) / values[i];
