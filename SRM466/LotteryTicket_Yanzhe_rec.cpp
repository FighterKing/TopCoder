...
class LotteryTicket {
private:
	bool canBuy(int price, vector<int> notes) {
		if (price < 0) return false;
		if (price == 0) return true;
		if (notes.size() == 0) return false;

		vector<int> bak {notes.begin(), notes.end()};
		for (auto n : bak) {
			notes.erase(find(notes.begin(), notes.end(), n));
			if (canBuy(price - n, notes)) return true;
			if (canBuy(price, notes)) return true;
		}
		return false;
	}
public:
	string buy(int price, int b1, int b2, int b3, int b4) {
		return canBuy(price, vector<int> {b1, b2, b3, b4}) ? "POSSIBLE" : "IMPOSSIBLE";
	}
};
