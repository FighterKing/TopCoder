...
public:
  int determineProfit(vector <int> profit, vector <int> decay) {
    ...
    auto d = min_element(decay.begin(), decay.end());
    if (p - profit.begin() != d - decay.begin())
      return (*p) - (*d);
    
    int pv = *p;
    int dv = *d;
    profit.erase(p);
    p = max_element(profit.begin(), profit.end());
    decay.erase(d);
    d = min_element(decay.begin(), decay.end());
    return max(pv - *d, *p - dv);
  }
};
